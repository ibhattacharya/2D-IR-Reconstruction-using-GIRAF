...
%% Set model order
res = [41,41];  %output resolution (use odd nums)
filter_siz = [17,17]; %filter dimensions (use odd nums)
...
%% run alg.
lambda = 1e7;  %regularization parameter
%parameters for epsilon update: eps = min(eps,gamma*s(r+1));
eps = 1;       %epsilson initialization
gamma = 5;     %epsilon multiplier for update
r = prod(filter_siz)-10;       %rank cut-off value
...
for j=1:length(s)
    ...
    mu = mu + (1/max(s(j),eps))*(abs(ifft2(filter)).^2);
end
...
[x,flag,relres] = pcg(R,y,1e-10,1000,[],[],x(:)); 
